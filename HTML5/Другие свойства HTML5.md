...
Самая большая проблема при использовании [[cookie]] заключается в том, что в каждом из них можно хранить максимум 4 Кбайт данных. Cookie также должны курсировать в обоих направлениях при каждой перезагрузке страницы. И, если только ваш сервер не использует шифрование на уровне передачи данных [[TLS (Transport Layer Security)|Transport Layer Security (TLS)]], являющееся более безопасным потомком [[SSL|SSL-шифрования (Secure Sockets Layer)]], при каждой передаче cookie путешествуют в открытом виде.
