---
title: CI_CD
date of creation: 2025-04-14T02:59:00
tags:
  - DevOPS
  - Developing
  - Backend
  - IT/Developing
  - Tests
  - Terminology
  - IT
  - IT/Terminology
  - Agile
read status: true
completion status: false
aliases:
  - CI/CD
  - Continuous  Integration/Continuous Delivery
---
# CI/CD 
![[CI_CD_diagramm.jpg]]

---

**CI (Continuous  Integration)** - Непрерывная интеграция.
**CD (Continuous Delivery)** - Непрерывная доставка.

**CI/CD** — ==это автоматизация тестирования и доставки новых модулей ПО конечным пользователям. CI/CD относиться к [[Agile]]-методологиям.== (Так же можно сказать что CI/CD — ==это современная аналогия конвейерного производства.==)

**CI/CD** - Это подход в которым мы выстаиваем процесс разработки программ так чтобы мы можно было бы вливать свой код с новыми фичами в проект который ты делаешь, а затем всё это должно собираться, тестироваться и развертываться.
